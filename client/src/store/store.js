...
export default new Vuex.Store({
	...
	actions: {
		...
		updatePost({ commit }, data) {
			...
			axios
				.put(`${process.env.VUE_APP_API_ENDPOINT}posts/${id}`, data, {
					headers: { Authorization: this.state.token }
				})
				.then((response) => {
					const post = response.data;
		...
		likePost({ commit }, payload) {
			axios
				.post(`${process.env.VUE_APP_API_ENDPOINT}posts/${payload.id}/like`, payload.data, {
					headers: { Authorization: this.state.token }
				})
				.then((response) => {
		...
		commentPost({ commit }, payload) {
			axios
				.post(`${process.env.VUE_APP_API_ENDPOINT}posts/${payload.id}/comments`, payload.data, {
					headers: { Authorization: this.state.token }
				})
				.then((response) => {
